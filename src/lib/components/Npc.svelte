<script lang="ts">
	...
	const managed: ManagedAgent = untrack(() => {
		...
		if (obj.ageFrac != null) m.ageFrac = obj.ageFrac; // restore exact age → sim set_age at spawn (adults stay adult)
		return m;
	});
	$effect(() => {
	...
	const PREG_GROW_SECS = math.personGestation(); // belly swells from first-trimester to full over the REAL gestation (from Rust)
